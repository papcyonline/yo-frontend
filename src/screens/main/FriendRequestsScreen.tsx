...
const FriendRequestsScreen: React.FC<FriendRequestsScreenProps> = ({ navigation }) => {
  ...
  const { refetch: refetchFriendRequestsCount } = useFriendRequests();
  ...
  const handleAcceptRequest = async (requestId: string) => {
    try {
      ...
      refetchFriendRequestsCount();
      
      // Record significant event for rating system
      try {
        await ratingService.recordSignificantEvent('friend_request_accepted');
      } catch (ratingError) {
